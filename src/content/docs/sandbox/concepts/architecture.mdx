...
1. **Client SDK** validates parameters and sends request to Durable Object via RPC
...
The Durable Object communicates with the container using one of two transport modes:

- **HTTP (default)** - Each SDK operation (file read, command execution, etc.) is a separate HTTP request to the container. Simple and reliable.
- **WebSocket** - All SDK operations multiplex over a single persistent WebSocket connection. Reduces sub-request count when running inside Workers or Durable Objects.

Configure the transport mode using the `transport` option:

```typescript
const sandbox = getSandbox(env.Sandbox, "my-sandbox", {
  transport: 'websocket'  // Multiplex over single connection
});
```

Refer to [Sandbox options](/sandbox/configuration/sandbox-options/#transport) for details on when to use each transport mode.
